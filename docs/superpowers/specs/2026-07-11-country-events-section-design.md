...
Alcance: solo la rama de `CountryPage` que ya renderiza recursos directamente (países sin selector de ciudad — `hasCitySelector === false`). La rama con `CityList` no se toca.

## Diseño

### Estilo visual (validado con mockups)

Agenda simple: una fila por evento con un chip de fecha (día grande + mes abreviado) a la izquierda, nombre del evento, y una línea de metadatos (ciudad · categoría) debajo — mismo lenguaje visual que el resto del sitio (`bg-caribe/10`, `text-caribe`, radios y tipografía de `ResourceLink`). Se descartaron una franja horizontal de días estilo carrusel y un mini calendario mensual navegable: ambas añaden estado/interacción (selección de día, navegación entre meses) que no se justifica todavía — no sabemos si un país va a tener más de un puñado de eventos simultáneos.
...
- `lib/locale-content.test.ts` (nuevo): casos para `formatEventBadge` — fecha simple en `es`/`en`, verificar formato de mes abreviado por locale.
