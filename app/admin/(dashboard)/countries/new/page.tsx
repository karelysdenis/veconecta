import { redirect } from 'next/navigation'
import { getSession } from '@/lib/lucia'
import { prisma } from '@/lib/prisma'
import { revalidatePath } from 'next/cache'
import Link from 'next/link'

export default async function NewCountryPage() {
  const { user } = await getSession()
  if (!user) redirect('/admin/login')
  if (user.role !== 'ADMIN') redirect('/admin')

  async function create(fd: FormData) {
    'use server'
    const { user } = await getSession()
    if (!user || user.role !== 'ADMIN') return
    await prisma.country.create({
      data: {
        slug: (fd.get('slug') as string).trim().toLowerCase(),
        nameEs: (fd.get('nameEs') as string).trim(),
        nameEn: (fd.get('nameEn') as string).trim(),
        namePt: (fd.get('namePt') as string).trim() || null,
        flag: (fd.get('flag') as string).trim(),
        active: fd.get('active') === 'on',
      },
    })
    revalidatePath('/admin')
    revalidatePath('/es')
    revalidatePath('/en')
    redirect('/admin')
  }

  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <Link href="/admin" className="text-sm text-gray-500 hover:underline">
          ← Inicio
        </Link>
      </div>

      <h1 className="text-xl font-bold text-gray-900 mb-6">Nuevo país</h1>

      <form action={create} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <F label="Slug (URL)" name="slug" required placeholder="ej: chile" />
          <F label="Bandera (emoji)" name="flag" required placeholder="🇨🇱" />
        </div>

        <F label="Nombre en español" name="nameEs" required placeholder="Chile" />
        <F label="Nombre en inglés" name="nameEn" required placeholder="Chile" />
        <F label="Nombre en portugués (opcional)" name="namePt" placeholder="Chile" />

        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" name="active" defaultChecked className="h-4 w-4 rounded" />
          <span className="text-sm text-gray-700">Visible en el selector público</span>
        </label>

        <div className="flex justify-end gap-3 pt-2">
          <Link href="/admin" className="text-sm text-gray-600 hover:underline px-4 py-2">
            Cancelar
          </Link>
          <button
            type="submit"
            className="bg-red-700 text-white text-sm font-medium px-5 py-2 rounded-lg hover:bg-red-800"
          >
            Crear país
          </button>
        </div>
      </form>
    </div>
  )
}

function F({
  label, name, required = false, placeholder = '',
}: {
  label: string; name: string; required?: boolean; placeholder?: string
}) {
  return (
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
      <input
        type="text"
        name={name}
        required={required}
        placeholder={placeholder}
        className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-300"
      />
    </div>
  )
}
